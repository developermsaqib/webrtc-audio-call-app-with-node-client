...
async function handleAudioStream(socket, data) {
  try {
    ...
    const sampleRate = 48000;
    const samplesPerFrame = 480;
    const playbackSpeed = 1.75;  // Adjust playback speed
    const frameInterval = 7; // 7ms for each frame
    ...
    // **Preload the first 6 seconds of audio to ensure the start is not missed**
    const preloadDuration = 6; // Preload 6 seconds
    const preloadSamples = preloadDuration * sampleRate;

    console.log("⚡ Preloading the first 6 seconds of audio...");

    // Preload the initial 6 seconds into output buffer
    for (let i = 0; i < preloadSamples; i++) {
      if (i < audioData.length) {
        outputBuffer[i % samplesPerFrame] = audioData[i]; // Fill buffer with samples
    ...
    console.log("✅ Preloading complete. Starting real-time playback...");

    // Start actual playback after preloading
    currentIndex = preloadSamples; // Set currentIndex to the correct position after preload
    ...
    socket.audioInterval = setInterval(() => {
      ...
      for (let i = 0; i < samplesPerFrame; i++) {
        ...
        outputBuffer[i] = audioData[sourceIndex] || 0;
      }
